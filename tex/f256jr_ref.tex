\documentclass{book}
\usepackage{datetime2}
\DTMsetdatestyle{ddmmyyyy}
\usepackage{hyperref}
\usepackage{xcolor}
\hypersetup{colorlinks=true,linkcolor=black}
\usepackage{amssymb}
\usepackage{notomath}
\usepackage[T1]{fontenc}
% \usepackage[paperwidth=8.5in, paperheight=5.5in,inner=1.5in,outer=1.0in,top=0.75in,bottom=0.75in,twoside]{geometry}
\usepackage[paperwidth=9.35in, paperheight=6.05in,inner=1.5in,outer=1.0in,top=0.75in,bottom=0.75in,twoside]{geometry}
% \usepackage[paperwidth=10.2in, paperheight=6.6in,inner=1.5in,outer=1.0in,top=0.75in,bottom=0.75in,twoside]{geometry}
\usepackage{multirow}
\usepackage{graphbox}
\usepackage{sectsty}
\usepackage[Bjornstrup]{fncychap} % Bjornstrup Glenn
\usepackage{framed}
\definecolor{darkblue}{rgb}{0.1, 0.0, 0.6}
\definecolor{silver}{rgb}{0.85, 0.85, 0.85}
\ChNumVar{\color{darkblue}\fontsize{55}{60}\usefont{OT1}{pzc}{m}{n}\selectfont}
\ChTitleVar{\color{darkblue}\raggedleft\LARGE\sffamily\bfseries}

\allsectionsfont{\color{darkblue}\bfseries\sffamily}
\newcommand{\jr}{F256jr}
\newcommand{\microsec}{$\mu$s}
\newcommand{\example}[1]{\subsection*{Example: #1}}
\begin{document}

\begin{titlepage}
    \colorbox{silver}{\makebox[\textwidth][r]{
    \shortstack{
        \vspace{3cm} \\
        \color{darkblue}\bfseries\sffamily\Huge Foenix \jr\ Reference Manual}} \\
    }

    \vfill

    \hfill\mbox{\color{darkblue}\bfseries\sffamily\Large Peter Weingartner}

    \hfill\mbox{\color{darkblue}\bfseries\sffamily\large \today}
\end{titlepage}

\tableofcontents
\listoftables

\input{introduction.tex}
\input{memory.tex}
\input{text.tex}
\input{graphics.tex}
\input{bitmap.tex}
\input{sprites.tex}
\input{tiles.tex}
\input{vicky_misc.tex}
\input{sound.tex}
\input{interrupts.tex}
\input{time.tex}
\input{via.tex}
\input{sdc.tex}
\input{kbd_mouse.tex}
\input{uart.tex}
\input{dma.tex}
\input{sysctrl.tex}
\input{iec.tex}
\input{math.tex}
\input{debug.tex}
\input{expansion.tex}
\input{connectors.tex}
\input{memory_map.tex}
\input{kernel.tex}
\input{links.tex}
\input{copyright.tex}

\end{document}
