...
There are two models of \jr\ available: the \fjr, and the \fk. The \fjr\ is sold as a single board in the mini-ITX format, which may be installed in a standard PC case and requires a separate PS/2 keyboard. The \fk\ is a complete system with a case and a built-in matrixed keyboard. The two versions do have differences but are mostly compatible. This manual was started when the \fjr\ was the only model and is mostly written from its perspective with differences in the \fk\ called out as exceptions.
